...
Call print_matrix("Result", res(), RR%, RC%)

GoTo enter_command

Sub main_add_scaler (mat(), rows%, cols%)
    add = in_val("Enter Scaler Addant: ")
...
Sub main_mult_scaler (mat(), rows%, cols%)
    mult = in_val("Enter Scaler Multiplier: ")
...
Sub main_add_matrices (mat(), rows%, cols%)
    ...
    lb
    a_scale! = in_float!("Enter " + matAName$ + " entries scale: ")
    b_scale! = in_float!("Enter " + matBName$ + " entries scale: ")
...
Sub input_matrix (matrix_name$, matrix_arr(), rows%, cols%)
    ...
    For i% = 1 To rows%
        pl "# ROW " + Str$(i%)
        For j% = 1 To cols%
            e = in_val(matrix_name$ + " " + format_ij$(i%, j%, ",") + " : ")
